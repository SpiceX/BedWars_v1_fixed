...
use BedWars\{BedWars, Entityfbw\EntityManagerfbw, Entityfbw\typesfbw\HumanEntityfbw, Entityfbw\typesfbw\TopsEntityfbw};
					...
					if ($sender->isOp()) {
						if (!empty($args[1])) {
							switch ($args[1]) {
								...
								case 'removewins':
									foreach ($sender->getLevel()->getEntities() as $entity) {
										if ($entity instanceof TopsEntityfbw) {
											$entity->kill();
										}
									}
									break;
								case 'removekills':
									foreach ($sender->getLevel()->getEntities() as $entity) {
										if ($entity instanceof TopsEntityfbwkills) {
											$entity->kill();
										}
									}
									break;
